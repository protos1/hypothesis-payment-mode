<template>
  <section class="s1-U__width--900px" style="margin: 0 auto;">
    <header class="s1-U__pd--bt32 s1-U__pd--tp32">
      <p
        class="md-caption s1-U__mg--lt48 s1-U__pd--lt4"
        v-if="Profile.Form.Id"
      >{{getNameById(StoreData.Profile, Profile.Form.Id)}}</p>
      <div class="s1-U__align-children--center">
        <md-button
          class="md-icon-button s1-U__mg--rt8"
          @click="Profile.DiscardEditingInterface = true"
        >
          <md-icon>arrow_back</md-icon>
        </md-button>
        <div>
          <h1 class="md-display-1">Profile editing</h1>
        </div>
      </div>
    </header>
    <md-card>
      <div
        class="md-layout md-alignment-center-center s1-loc__loading"
        :class="Profile.EditingFormLoading && 'active'"
      >
        <md-progress-spinner md-mode="indeterminate"></md-progress-spinner>
      </div>
      <md-content class="md-scrollbar s1-U__pd16 s1-U__pd--lt48" style="overflow: auto">
        <profile-form :StoreData="StoreData" :Profile="Profile" :$v="$v"></profile-form>
      </md-content>
      <md-card-actions class="s1-U__pd16 s1-U__border--top1 s1-U__flex-shrink-0">
        <md-button
          class="md-primary md-raised"
          :disabled="$v.Profile.Form.$invalid"
          @click="saveEdited('Profile')"
        >
          <span class="s1-U__pd--lt8 s1-U__pd--rt8">Save changes</span>
        </md-button>
      </md-card-actions>
    </md-card>

    <md-dialog-confirm
      :md-active.sync="Profile.DiscardEditingInterface"
      md-title="Discard profile editing?"
      md-content="When you discard, the profile information edited will be discarded."
      md-confirm-text="discard"
      md-cancel-text="back"
      @md-cancel="Profile.DiscardEditingInterface = false"
      @md-confirm="discardEditing('Profile')"
    />
  </section>
</template>

<script>
import ProfileForm from "./ProfileForm.vue";

export default {
  name: "ProfileEditing",
  components: {
    ProfileForm
  },
  props: {
    Profile: Object,
    StoreData: Object,
    $v: Object,
    saveEdited: Function,
    discardEditing: Function
  }
};
</script>